package ru.ktsstudio.app_verification.ui.object_survey.product
...
import ru.ktsstudio.app_verification.di.object_survey.product.ProductSurveyComponent
...
import ru.ktsstudio.app_verification.presentation.object_survey.equipment.production.ProductSurveyUiState
 ...
 */
class ProductSurveyFragment : Fragment(R.layout.fragment_survey), ConfirmDialogFragment.OnClickListener {

    private val args: ProductSurveyFragmentArgs by navArgs()

    @Inject
    lateinit var factory: ViewModelProvider.Factory

    private val binding: FragmentSurveyBinding by viewBinding(FragmentSurveyBinding::bind)
    private var surveyAdapter: SurveyAdapter<ProducedEntityType> by autoCleared()

    private val viewModel: ProductSurveyViewModel by viewModels { factory }
    private val mediaSurveyViewModel: @JvmSuppressWildcards MediaSurveyViewModel<ProductionSurveyDraft>
        by viewModels { factory }
    private val exitSurveyViewModel: @JvmSuppressWildcards ExitSurveyViewModel<TechnicalSurveyDraft>
        by viewModels { factory }

    private lateinit var mediaCaptureDelegate: MediaCaptureDelegate<ProductionSurveyDraft>
    private lateinit var exitSurveyDelegate: ExitSurveyDelegate<TechnicalSurveyDraft>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        ProductSurveyComponent.create(args.objectId).inject(this)
    }
    ...
    private fun renderState(state: ProductSurveyUiState) {
