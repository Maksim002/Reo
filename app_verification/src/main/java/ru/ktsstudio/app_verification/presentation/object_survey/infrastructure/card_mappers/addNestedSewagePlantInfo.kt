...
fun MutableList<Any>.addNestedSewagePlantInfo(
    sewagePlant: SewagePlant,
    types: List<Reference>,
    resources: ResourceManager
) {
    val lastIndex = sewagePlant.equipment.values.size - 1
    add(
        InnerMediumTitle(
    ...
    add(
        InnerLabeledEditItem(
            ...
            valueConsumer = StringValueConsumer<InfrastructureSurveyDraft>(
                value = sewagePlant.count?.toString().orEmpty()
            ) { value, updatable ->
    ...
    add(EmptySpace(isNested = true))
    sewagePlant.equipment.values.forEachIndexed { index, equipment ->
        val isLast = index == lastIndex
        add(CardCornersItem(isTop = true, isNested = true))
        addNestedSewagePlantEquipment(
            equipment = equipment,
            resources = resources,
            types = types,
        ...
        add(
            DeleteEntityItem(
                id = equipment.id,
                inCard = true,
                qualifier = InfrastructureEquipmentType.SEWAGE_PLANT
            )
        )
